...
const Page = () => {
    ...
    const fetchSchemes = async (page = 1, keyword = "") => {
        setLoading(true);
        try {
            const response = await AppApi.fetchSchemes(
                page,
            ...
            if (response.success && response.data.data) {
                setSchemes(
                    response.data.data.map((scheme: any) => ({
                        id: scheme.id,
                        title: scheme.schemeName,
                        state: Array.isArray(scheme.beneficiaryState)
    ...
    return (
        <div className="min-h-screen w-full bg-white">
            ...
            <div className="bg-[#fffcf1] p-8">
                <h1 className="text-3xl font-bold text-[#bf8b2e] w-1/2 mb-2">
                    {value}
                </h1>
            </div>
            ...
            <div className="p-4 mx-auto">
                    ...
                    {loading ? (
                        ...
                        schemes.map((scheme) => (
                            <Link
                                href={
                                    scheme.scheme_url ||
                                    `/scheme/id/detail?id=${scheme.id}`
                                }
                                target={scheme.scheme_url ? "_blank" : "_self"}
                                key={scheme.id}
                                className="block"
                            >
                        ...
                        <div className="text-center py-8">
                            <p>
                                No schemes found. Try a different search query.
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="mt-6 flex justify-center">
                    ...
                    <span className="px-4 py-2 mx-1 bg-gray-200 rounded-md">
                        {currentPage}
                    </span>
                    {totalPages > 0 && (
                        <span className="px-4 py-2 mx-1 text-gray-500">
                            of {totalPages}
                        </span>
                    )}
                    <button
