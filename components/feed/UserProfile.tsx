            ...
            <li>
                <div className="flex flex-col">
                    {isMobile ? (
                        ...
                        >
                            <LogOut className='m-0'/>
                            <span className="text-md font-regular ml-2 hidden lg:block">Logout</span>
                        </button>
                    )}
                </div>
            </li>
    ...
    const renderUserProfile = () => (
        <div className="flex-col items-center md:items-start border-b py-2 border-gray-600 flex">
            <div className="relative mb-2 w-20">
                ...
                <button aria-label='online status' className="absolute bottom-0 right-0 w-6 h-6 rounded-full bg-white border border-gray-200 flex items-center justify-center shadow-sm">
            ...
            <div className="block md:hidden">
                <Sheet>
                    <SheetTrigger asChild>
                        <button aria-label='open-menu' className="p-2 cursor-pointer rounded-lg hover:bg-gray-100">
                            <Image src='/Logo.svg' alt='SDG Logo' width={35} height={35} />
                        </button>
                    </SheetTrigger>
                    <SheetContent side="left" className="h-full rounded-r-xl w-fit  md:hidden">
                        <SheetHeader className="mb-4">
                            <SheetTitle className="flex items-center justify-center">
                                <Image src='/Logo.svg' alt='SDG Logo' width={35} height={35} />
                                <span className="ml-2 text-lg font-bold">The SDG Story</span>
                            </SheetTitle>
                        </SheetHeader>

                        {renderUserProfile()}
                        
                        <nav className="mt-4 w-fit">
                            {renderNavItems(true)}
                        </nav>
                    </SheetContent>
                </Sheet>
...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon, label, isActive, route, showLabel, isMobile }) => {
  const linkContent = (
    <Link
      href={route} 
      className={`flex items-center justify-start space-x-2 py-2 p-2 rounded-xl hover:bg-accent/30 ${isActive ? ' font-bold text-accent' : ''}`}
    >
      <Image src={icon} alt={label} className='object-fill m-0' width={25} height={25} />
      <span className={`text-md font-regular ml-2 block md:hidden lg:block`}>{label}</span>
    </Link>
  );
