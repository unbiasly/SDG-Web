...
const VideoPageClient = ({ videoId }: { videoId: string }) => {
    ...
    const [commentSubmitted, setCommentSubmitted] = useState(false); // Flag for new comments
    ...
    const handleCommentSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setCommentText("");
                setCommentSubmitted(prev => !prev); // Toggle the flag to trigger useEffect
            }
        } catch (error) {
        ...
        <div className="flex-col w-full max-w-3xl m-auto px-2 sm:px-4 py-3 sm:py-6 md:px-0 flex gap-2 sm:gap-4">
            <div className="sticky top-0 bg-white z-10 flex items-center p-2 sm:p-3 border-b border-gray-200">
                <Link
                    ...
                    className="mr-2 sm:mr-4 p-1 sm:p-0"
                >
                    ...
                    <div className="flex items-start gap-2 sm:gap-3 mb-4 sm:mb-6">
                        ...
                        <div className="flex-1 flex flex-row sm:items-center gap-2">
                            <input
                                ...
                                className="flex-1 w-full bg-transparent border p-2 px-3 border-gray-300 focus:outline-none focus:border-accent rounded-full text-sm sm:text-base"
                                value={commentText}
                                onChange={(e) => setCommentText(e.target.value)}
                                onKeyPress={handleCommentKeyPress}
                            />
                            <Button
                                className="ml-auto sm:ml-2 text-sm sm:text-base py-2 px-3 sm:px-4 hover:bg-accent rounded-full"
