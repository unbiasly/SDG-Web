...
export default function GoalHeader({ goal }: GoalProps) {
  ...
  return (
    <div className={`rounded-2xl flex flex-col justify-between md:h-[75vh] h-[60vh] p-3 sm:p-5 text-white relative bg-[${goal.color}]`}>
      {/* Navigation buttons at the top with higher z-index */}
      <div className="flex justify-between mb-4 sm:mb-8 z-20 relative">
        <div>
          {prevGoal && (
            <Link 
              href={`/goals/${prevSlug}`} 
      ...
      {/* Content section with flex-grow to take available space */}
      <div className="flex-grow flex items-center px-4 sm:px-8 md:px-16 relative z-10">
        <div className="flex items-start justify-start space-x-3 sm:space-x-6">
          ...
          <div className="flex flex-col space-y-2 sm:space-y-4 md:space-y-6">
              ...
              <p className="text-xs sm:text-xl md:text-2xl font-bold">
              {goal.subtitle}
              </p>
          </div>
        </div>
      ...
      {/* Image as absolute positioned overlay */}
      <div className={`absolute bottom-0 left-0 w-full ${goal.id === 5 ? 'h-1/3' : 'h-full'} z-0`}>
        <Image 
